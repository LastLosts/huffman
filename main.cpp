...
static void print_separators(uint32_t level)
{
...
// static void print_tree(Node *root, uint32_t counter = 0)
// {
//     uint32_t depth = get_tree_depth(root);
//
//     if (root->ch != '\0')
//         std::cout << (char)root->ch << "\n";
//     else
//         std::cout << ' ' << "\n";
//     print_separators(counter);
//
...
static void encode_chunk(const char *data, uint32_t data_size, std::vector<char> &out_data, NodeEncoded *huffman_codes)
{
    ...
    for (uint32_t i = 0; i < data_size; ++i)
    {
        ...
        else
        {
            ...
            std::cout << "push_back(packed) ";
            print_binary({uint64_t(byte + 128), 8});
            std::cout << "\n";
...
static void encode(const char *output_filename, NodeEncoded *huffman_codes, const char *input_filename)
{
    ...
    while (true)
    {
        ...
        out_file.write(encoded.data(), encoded.size());
    }
    in_file.close();
    out_file.close();
}
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Compressed from " << file_size << "B to " << total_byte_length_encoded << "B\n";
    // print_file_binary("./output.huff");
